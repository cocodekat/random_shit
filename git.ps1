...
# Get ip
# Get all IPv4 addresses except APIPA (169.254.*) and loopback
...
# Write all found IPs to the log, or NOT FOUND if none
if ($ips.Count -gt 0) {
    foreach ($ip in $ips) {
        Add-Content -Path $LogFilePath -Value "IP: $ip"
    }
} else {
    Add-Content -Path $LogFilePath -Value "IP: NOT FOUND"
}
